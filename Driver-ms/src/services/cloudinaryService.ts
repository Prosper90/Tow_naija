...
class CloudinaryService {
  ...
  private generatePath(driverId: string, fileName: string): string {
    return `TowNiaja/Drivers/${driverId}/${fileName}`;
  }
  ...
  async uploadDriverImage(
    ...
    driverId: string,
    imageType: 'avatar' | 'license_front' | 'license_back' | 'car_plate' | 'registration_cert'
  ...
  async deleteDriverImage(publicId: string): Promise<boolean> {
    try {
  ...
  async uploadMultipleDriverImages(
    driverId: string,
    files: {
    ...
    const imageTypeMap = {
      'avatar': 'avatar',
      'driver_lisence_img_front': 'license_front',
      'driver_lisence_img_back': 'license_back',
      'car_pic_with_platenumber': 'car_plate',
      'vehicle_reg_image_cert': 'registration_cert'
    };

    for (const [fieldName, fileArray] of Object.entries(files)) {
      if (fileArray && fileArray.length > 0) {
        const imageType = imageTypeMap[fieldName];
        uploads[imageType] = await this.uploadDriverImage(
          fileArray[0],
          driverId,
          imageType as any
        );

...
class BaseRepository<T extends Document> {
  ...
  public async findOneByFieldAndPopulate(field: { [key: string]: any; }, populateFields: string[]): Promise<T | null> {
    try {
      return (await this.model.findOne(field as FilterQuery<T>)).populate(populateFields);
    } catch (error) {
      throw error;
    }
  }
